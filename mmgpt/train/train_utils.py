...
def train_one_epoch(
    ...
    media_token_id = tokenizer(
        '<image>', add_special_tokens=False)['input_ids'][-1]
    endofchunk_token_id = tokenizer(
        '<|endofchunk|>', add_special_tokens=False)['input_ids'][-1]
    ...
    for num_steps, (batch_laion, batch_mmc4) in tqdm(
        ...
        # C4 FORWARD PASS
        images = batch_mmc4[0].to(
            device_id, dtype=cast_dtype, non_blocking=True).unsqueeze(2)
        ...
        for i in range(labels.shape[0]):
            ...
            label_idx = 0
            while label_idx < labels.shape[1] and labels[i][
                    label_idx] != media_token_id:
            ...
            for endofchunk_idx in endofchunk_idxs:
                ...
                while token_idx < labels.shape[1] and labels[i][
                        token_idx] != media_token_id:
        ...
        if (((num_steps + 1) % args.gradient_accumulation_steps)
            ...
            if args.rank == 0 and args.report_to_wandb:
                ...
                laion_samples_per_second = (
                    args.gradient_accumulation_steps * args.batch_size_laion *
