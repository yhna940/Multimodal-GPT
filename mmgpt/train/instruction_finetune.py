...
from mmgpt.train.distributed import (init_distributed_device,
                                     world_info_from_env)
from mmgpt.train.train_utils import (AverageMeter, get_autocast,
                                     get_cast_dtype, get_checkpoint)
...
def main():
    ...
    parser.add_argument(
        '--lm_path', default='checkpoints/llama-7b_hf', type=str)
    parser.add_argument(
        '--tokenizer_path',
    ...
    parser.add_argument(
        '--use_media_placement_augmentation', action='store_true')
    ...
    # wandb args
    parser.add_argument(
        '--report_to_wandb', default=False, action='store_true')
    parser.add_argument(
        '--wandb_project',
        type=str,
    )
    ...
    model, image_processor, tokenizer = create_model_and_transforms(
        ...
        if args.tokenizer_path else args.lm_path,
    ...
    if dataset_config.get('language_datasets') is not None and len(
            dataset_config.language_datasets) > 0:
    ...
    if os.path.exists(
            f'{args.run_name}') and args.resume_from_checkpoint is None:
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        if args.rank == 0:
            ...
            print(
                f'Saving checkpoint to {args.run_name}/checkpoint_{epoch}.pt')
...
def train_one_epoch(
    ...
    for num_steps, batch in tqdm(
        ...
        if language_dataloader is not None:
            ...
            lang_input_ids = batch_lang['input_ids'].to(
                device_id, dtype=cast_dtype, non_blocking=True)
            lang_attention_mask = batch_lang['attention_mask'].to(
                device_id, dtype=cast_dtype, non_blocking=True)
            lang_labels = batch_lang['labels'].to(
                device_id, dtype=cast_dtype, non_blocking=True)
        ...
        if (((num_steps + 1) % args.gradient_accumulation_steps)
                == 0) or (num_steps == num_batches_per_epoch - 1):
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
